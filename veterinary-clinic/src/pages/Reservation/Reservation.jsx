...
const Reservation = () => {
	...
	const formBookingReducer = (state, action) => {
		switch (action.type) {
			case "UPDATE_FIELD":
				const newValue =
					action.field === "fecha" && action.value instanceof Date
						? action.value.toISOString()
						: action.value;
				return {
					...state,
					[action.section]: {
			...
			axios
				...
				.then((response) => {
					...
					const formattedReservations =
						formatReservationsForDatePicker(reservations);
	...
	const [excludeTimesForSelectedDate, setExcludeTimesForSelectedDate] =
		useState([]);
					...
					<form className="booking-form" ref={form}>
						...
						<div className="booking-info">
							...
							<select
								...
								onChange={(e) =>
									handleInputChange("dataMascota", "id", e.target.value)
								}
							>
								<option value={1} disabled>
